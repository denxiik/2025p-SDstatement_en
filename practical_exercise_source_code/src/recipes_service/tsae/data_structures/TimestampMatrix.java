...
public class TimestampMatrix implements Serializable{
	...
	public void updateMax(TimestampMatrix tsMatrix){
		for (String hostId : tsMatrix.timestampMatrix.keySet()) {
            TimestampVector otherVector = tsMatrix.timestampMatrix.get(hostId);
            TimestampVector localVector = timestampMatrix.get(hostId);
            if (otherVector != null && localVector != null) {
                localVector.updateMax(otherVector);
            } else if(localVector == null){
                timestampMatrix.put(hostId, otherVector);
            }
        }
	}
	...
	public TimestampVector minTimestampVector(){
        ...
        List<String> participants = new ArrayList<>(timestampMatrix.keySet());
        TimestampVector minVector = new TimestampVector(participants);
        for (String participant : participants) {
            TimestampVector currentVector = timestampMatrix.get(participant);
            if (currentVector != null) {
                minVector.mergeMin(currentVector);
            }
        }
        return minVector;
	}
	...
	public TimestampMatrix clone(){
        ...
        for (String hostId : timestampMatrix.keySet()) {
            clone.timestampMatrix.put(hostId, timestampMatrix.get(hostId).clone());
        }
        return clone;
	}
